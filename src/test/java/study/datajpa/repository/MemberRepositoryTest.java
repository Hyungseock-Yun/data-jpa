...
//    assertThat(page.getTotalPages()).isEqualTo(2);
  ...
  public void queryHint() {
    ...
    // when
//    Member member = memberRepository.findById(member1.getId()).get();     // 실무에서는 get 바로 쓰면 안됨.
    Member member = memberRepository.findReadOnlyByUsername("member1");
    member.setUsername("member2");

    em.flush();   // 기존 member1 객체와 member2 객체를 둘 다 가지고 있어 비효율적.
  }
  ...
  public void queryByExample() {
    ...
    ExampleMatcher matcher = ExampleMatcher.matching()
      .withIgnorePaths("age");   // age의 경우 primitive 타입(int)이기 때문에 무시한다.
...
//    List<UsernameOnlyDto> result = memberRepository.findProjection("m1", UsernameOnlyDto.class);
